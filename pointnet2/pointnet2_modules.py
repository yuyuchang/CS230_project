...
class PointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        #print("grouped_features shape is:", grouped_features.shape)
...
class SVFE(nn.Module):
    ...
    def __init__(self, svfe):
        super(SVFE, self).__init__()
        self.vfe_1 = VFE(svfe[0], svfe[1])
        self.vfe_2 = VFE(svfe[1], svfe[2])
        self.fcn = FCN(svfe[2], svfe[3])
    def forward(self, x):
...
class VoxelPointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        B = grouped_features.shape[0]
        C = grouped_features.shape[1]
        grouped_features = grouped_features.view(-1, 1, C)
        new_features = self.SVFE(grouped_features).view(B, -1, self.npoint, self.nsample)
